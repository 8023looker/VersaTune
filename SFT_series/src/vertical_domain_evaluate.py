...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path_list: List, tokenizer: transformers.PreTrainedTokenizer, domain_weight: Optional[Dict] = None):
        ...
        list_data_dict = utils.jload(data_path_list, domain_weight, if_eval=True) # domain weight can be float(ratio) or int(sample number)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, batch_size, data_args, domain_weight) -> Dict:
    ...
    code_eval_dataset = SupervisedDataset(tokenizer=tokenizer, data_path_list=[file_path for file_path in data_args.data_path if file_path.split('/')[-2] == "codeAlpaca"], domain_weight=eval_domain_weight)
...
def evaluate_loss(domain_weight_dict, model_args, data_args, training_args): #(domain_weight_dict, model, tokenizer, data_module, training_args, data_args):
    ...
    eval_dataloader = {
        ...
        "law": get_dataloader(data_module["law_eval_dataset"], data_module["data_collator"], batch_size=1),
        "medical": get_dataloader(data_module["law_eval_dataset"], data_module["data_collator"], batch_size=1),
...
if __name__ == "__main__":
    ...
    # multi_ability_sft_choice = "versatune_dynamic"
    # prev_domain_weight_dict = utils.initialize_multi_ability_domain_weight(multi_ability_sft_choice, model_name, total_training_count=60000)
    params_dict = utils.get_params(params_folder_path + model_name + "_params.json") # read the updated model path
    model_args.model_name_or_path = params_dict["model_path"] # current model path!
    prev_domain_weight_dict = params_dict["domain_weight"]
    
    updated_domain_weight_dict = evaluate_loss(prev_domain_weight_dict, model_args, data_args, training_args)
    params_dict["domain_weight"] = updated_domain_weight_dict # update domain weight of param dict
    utils.write_params(params_folder_path + model_name + "_params.json", params_dict) # update domain_weight
    
    
    ## 能跑通的代码...
    # initial_topic_weight_path = "/cpfs/29f69eb5e2e60f26/user/sft_intern/keerlu/CPT_params/SFT_series/domain_infer/result/baichuanSEED_40k_code.json"
    # model_params_dict = utils.initialize_model_params(initial_topic_weight_path)
    ...
    ## utils.write2model_param(model_params_dict) # for debugging, not necessary