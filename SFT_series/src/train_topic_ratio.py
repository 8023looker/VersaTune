...
""" multi-ability SFT training src """
import os
os.environ["WANDB_MODE"] = "offline" # emmm 服务器连不上 wandb...
os.environ["CUDA_VISIBLE_DEVICES"]="0,1,2,3" # 在 bash 中设置
...
def find_model_subfolder(current_dir):
    # 获取当前目录下的所有子文件夹
    subdirectories = [name for name in os.listdir(current_dir) if os.path.isdir(os.path.join(current_dir, name))]
    # 过滤出符合条件的子文件夹
    filtered_subdirectories = [name for name in subdirectories if name.startswith("checkpoint-")]
    return filtered_subdirectories[0]


def train(domain_weight_dict, model_args, data_args, training_args):
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer, batch_size=training_args.per_device_eval_batch_size * torch.cuda.device_count(), data_args=data_args, domain_weight=domain_weight_dict)
    # construct the data_module for Trainer
    trainer_data_module = copy.deepcopy(data_module)
    trainer_data_module.pop("finance_eval_dataset")
    trainer_data_module.pop("law_eval_dataset")
    trainer_data_module.pop("medical_eval_dataset")
    trainer_data_module.pop("science_eval_dataset")
    trainer_data_module.pop("code_eval_dataset")
    trainer_data_module.pop("general_eval_dataset")
    
    trainer = Trainer(model=model, tokenizer=tokenizer, args=training_args, **trainer_data_module)
    trainer.train()
    trainer.save_state()
    trainer.save_model(output_dir=training_args.output_dir)
    
    # model_args.model_name_or_path = training_args.output_dir + find_model_subfolder(training_args.output_dir)
    model_args.model_name_or_path = training_args.output_dir # "/cpfs/29f69eb5e2e60f26/user/sft_intern/keerlu/HUAWEI/output/loss-gradient/" + training_args.output_dir
...
if __name__ == "__main__":
    ...
    # print("model_args", model_args.model_name_or_path.split('/')[-2])
    model_name = model_args.model_name_or_path.split('/')[-2]
    
    multi_ability_sft_choice = "versatune_constant" # "uniform", "reverse_knowledge", "versatune_constant", "versatune_dynamic"
    
    if multi_ability_sft_choice == "versatune_dynamic":
        params_folder_path = "/cpfs/29f69eb5e2e60f26/user/sft_intern/keerlu/CPT_params/SFT_series/src/multi_ability/params/"
        params_dict = utils.get_params(params_folder_path + model_name + "_params.json") # read the updated model path
        
        model_args.model_name_or_path = params_dict["model_path"] # current ckpt path
        domain_weight_dict = utils.initialize_multi_ability_domain_weight(multi_ability_sft_choice, model_name, total_training_count=60000)
        model_args = train(domain_weight_dict, model_args, data_args, training_args)
        
        # update model_name_or_path
        params_dict["model_path"] = model_args.model_name_or_path
        utils.write_params(params_folder_path + model_name + "_params.json", params_dict) # update ckpt_path
    else:
        domain_weight_dict = utils.initialize_multi_ability_domain_weight(multi_ability_sft_choice, model_name, total_training_count=60000)
        model_args = train(domain_weight_dict, model_args, data_args, training_args)

    
    # if multi_ability_sft_choice in ["uniform", "reverse_knowledge", "versatune_constant"]:
    #     domain_weight_dict = utils.initialize_multi_ability_domain_weight(multi_ability_sft_choice, model_name, total_training_count=60000)
    #     model_args = train(domain_weight_dict, model_args, data_args, training_args)
    # else if multi_ability_sft_choice == "versatune_dynamic":
    #     pass # 待补充，需要修改 model_name_or_path 的设置
    #     # update model_name_or_path
